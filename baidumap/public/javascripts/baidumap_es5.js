...
$('form').submit(function () {
    ...
    var createKeywordPanel = function createKeywordPanel(data) {
        this.data = data;
    ...
    var CreateSearchResultPanel = function CreateSearchResultPanel(data) {
        $('#searchrestult_panel').empty();
        $('#searchrestult_panel').css('display', 'block');
        var ul = $('<ul></ul>');
        for (var i = 0; i < data.length && i < 10; i++) {
...
map.centerAndZoom(undefined.city); // 初始化地图,设置中心点坐标和地图级别
//添加地图类型控件
...
map.setCurrentCity(undefined.city); // 设置地图显示的城市 此项是必须设置的
